...
export const OpenTimeSlots = () => {
	...
	const onFinish = async values => {
		...
		const filtered = timeSlots?.filter(timeSlot => {
			const time =
				timeSlot?.time.split(' ')[1] + ' ' + timeSlot?.time.split(' ')[2]
			const isInstructorValid = !values.staff || values.staff === timeSlot.staff
			const isLocationValid =
				!values.location || values.location === timeSlot.location
			const isVehicleValid =
				!values.vehicle || values.vehicle === timeSlot.vehicle
			const isWeekdaysValid =
				!values.weekdays ||
				values.weekdays?.every(day => timeSlot.week_range.includes(day))
			const isAppointmentType = !values.type || values.type === timeSlot?.type
			const isTimeFilter =
				!values.time || timeZone(time.split('-')[0], values.time)
			const isSelectDate =
				!values.date ||
				dayjs(values.date[0]).isSame(dayjs(timeSlot.date), 'day')
			return (
				isInstructorValid &&
				isLocationValid &&
				isVehicleValid &&
